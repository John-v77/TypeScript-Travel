...
jest.mock("../../controllers/authController", () => {
  ...
  return {
    ...
    default: {
      ...actual,
      // restrictTo(...) is NOT mocked, so the real implementation runs and
      // reads req.user.role — protect must set a user with an allowed role
      // (admin/lead-guide) or POST/PATCH routes crash on "Cannot read
      // properties of undefined (reading 'role')".
...
describe("Tour Routes", () => {
  ...
  describe("Virtual Fields", () => {
    it("should include virtual field durationWeeks in JSON response for single tour", async () => {
      ...
      // handlerFactory.getOne calls .populate(popOptions) on the findById
      // query, so the mock must be chainable, not a plain resolved value.
      ...
      expect(response.body.data.durationWeeks).toBe(2);
      expect(response.body.data.duration).toBe(14);
    });
    ...
    it("should include virtual field durationWeeks in JSON response for tour list", async () => {
      ...
      expect(response.body.data[0].durationWeeks).toBe(1);
      expect(response.body.data[1].durationWeeks).toBe(3);
    });
    ...
    it("should calculate durationWeeks correctly for different durations", async () => {
      ...
      expect(response.body.data.durationWeeks).toBeCloseTo(1.43, 2);
    });
    ...
    it("should include virtual field in updated tour response", async () => {
      ...
      expect(response.body.data.durationWeeks).toBe(4);
    });
  });
  ...
  describe("Document Middleware - Slugify", () => {
    it("should generate slug when creating a new tour", async () => {
      ...
      expect(response.body.data.slug).toBe("the-forest-hiker");
      expect(response.body.data.name).toBe("The Forest Hiker");
    });
    ...
    it("should generate slug with special characters removed", async () => {
      ...
      expect(response.body.data.slug).toBe("the-amazing-tour-adventure-fun");
    });
    ...
    it("should generate slug with spaces converted to hyphens", async () => {
      ...
      expect(response.body.data.slug).toBe("mountain-climbing-experience");
    });
    ...
    it("should generate lowercase slug", async () => {
      ...
      expect(response.body.data.slug).toBe("extreme-adventure-tour");
    });
    ...
    it("should update slug when tour name is updated", async () => {
      ...
      expect(response.body.data.slug).toBe("new-adventure-name");
      expect(response.body.data.name).toBe("New Adventure Name");
    });
    ...
    it("should handle numbers in tour name for slug generation", async () => {
      ...
      expect(response.body.data.slug).toBe("7-day-beach-adventure-2024");
    });
  });
  describe("Query Middleware - Secret Tour Filtering", () => {
    it("should hide secretTour field from tour responses", async () => {
      ...
      expect(response.body.data[0].secretTour).toBeUndefined();
      expect(response.body.data[1].secretTour).toBeUndefined();
      expect(response.body.data[0].name).toBe("Regular Tour");
      expect(response.body.data[1].name).toBe("Public Tour");
    });
    ...
    it("should hide secretTour field from single tour response", async () => {
      ...
      expect(response.body.data.secretTour).toBeUndefined();
      expect(response.body.data.name).toBe("Regular Tour");
    });
    ...
    it("should hide secretTour field from created tour response", async () => {
      ...
      expect(response.body.data.secretTour).toBeUndefined();
      expect(response.body.data.name).toBe("New Tour");
    });
    ...
    it("should hide secretTour field from updated tour response", async () => {
      ...
      expect(response.body.data.secretTour).toBeUndefined();
      expect(response.body.data.name).toBe("Updated Tour");
    });
    ...
    it("should automatically filter out secret tours from queries", async () => {
      ...
      expect(response.body.data[0].name).toBe("Public Tour");
      expect(response.body.data[0].secretTour).toBeUndefined();
    });
    ...
    it("should verify secretTour field exists in schema but is hidden", async () => {
      ...
      expect(response.body.data.secretTour).toBeUndefined();
      expect(response.body.data.name).toBe("Test Tour");
    });
  });
  ...
  describe("Price Discount Validation", () => {
    it("should accept valid discount percentages", async () => {
      ...
      for (const discount of validDiscounts) {
        ...
        expect(response.body.data.priceDiscount).toBe(discount);
        expect(response.body.data.name).toBe(`Tour with ${discount}% discount`);
      }
    });
    ...
    it("should allow tours without priceDiscount", async () => {
      ...
      expect(response.body.data.priceDiscount).toBeUndefined();
      expect(response.body.data.name).toBe("Tour without discount");
    });
    ...
    it("should validate discount when updating tour", async () => {
      ...
      expect(response.body.data.priceDiscount).toBe(15);
    });
  ...
  describe("Tour Model Validations", () => {
    describe("Rating Average Validation", () => {
      ...
      it("should accept valid rating between 1.0 and 5.0", async () => {
        ...
        expect(response.body.data.ratingsAverage).toBe(4.2);
      });
    ...
    describe("Tour Name Validation", () => {
      ...
      it("should accept valid tour name with only letters and spaces", async () => {
        ...
        expect(response.body.data.name).toBe("Amazing Adventure Tour");
      });
      ...
      it("should accept tour name with exactly 10 characters", async () => {
        ...
        expect(response.body.data.name).toBe("Short Tour");
      });
  ...
  describe("Combined Validation Tests", () => {
    ...
    it("should accept tour with all valid fields", async () => {
      ...
      expect(response.body.data.name).toBe("Perfect Adventure Tour");
      expect(response.body.data.ratingsAverage).toBe(4.7);
      expect(response.body.data.priceDiscount).toBe(15);
    });
  });
